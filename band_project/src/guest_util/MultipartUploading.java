...
public class MultipartUploading {
	...
	public MultipartUploading(HttpServletRequest request) 
									throws Exception{//积己磊
		FileItemFactory factory = new DiskFileItemFactory();//按眉积己
		ServletFileUpload upload = new ServletFileUpload(factory);//按眉积己
		items = upload.parseRequest(request);
		if(ServletFileUpload.isMultipartContent(request)) multiPart = true;
		parameterMap = new HashMap<String, String[]>();
		for(FileItem fileItem : items){
			String name = fileItem.getFieldName();
			if(fileItem.isFormField()){//input type = file阑 力寇茄 贸府
				String value = "";
				value = new String(
