...
import sch_model.IdSequence;
import sch_model.Writing;

public class CrudProcess {
	private final String namespace = "configurations.SchMapper";
	...
	public Integer deleteWritingInfo(int writingid){
		...
		try{
			int result = sqlSession.delete(
					namespace+".deleteWritingInfo", writingid);
			if(result > 0) sqlSession.commit();
			else sqlSession.rollback();
			return result;
		}finally{sqlSession.close();}
	...
	public Integer deleteWritingContent(int writingid){
		...
		try{
			int result = sqlSession.delete(
					namespace+".deleteWritingContent", writingid);
			if(result > 0) sqlSession.commit();
			else sqlSession.rollback();
			return result;
		}finally{sqlSession.close();}
